# -*- coding: utf-8 -*-
"""
slice_scratch.py - Deterministic verbatim block slicer for the ingestion scratchpad.

The sentinel-ingestion-report pipeline (Invoke-IngestionScan.ps1) writes pre-rendered
markdown tables into temp/ingest_scratch_<ts>.md under stable `### <Section>`
markers (e.g. SE_Computer, SE_EventID, SyslogHost, AnomalyTable, DailyChart,
CrossRef). When assembling the final report you must copy those tables VERBATIM
(the SKILL forbids recomputing volumes, renaming tables, or reordering rows).
...
Usage:
    python slice_scratch.py --scratch temp/ingest_scratch_<ts>.md --list
    python slice_scratch.py --scratch temp/ingest_scratch_<ts>.md --section SE_Computer
    python slice_scratch.py --scratch temp/ingest_scratch_<ts>.md --all
"""
import argparse
import io
import sys
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Verbatim slicer for the Sentinel ingestion scratchpad.")
    ap.add_argument("--scratch", required=True, help="Path to ingest_scratch_<ts>.md")
